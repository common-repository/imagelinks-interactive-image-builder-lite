...
class ImageLinks_Builder {
	...
	function shortcode($atts) {
		...
		if($item) {
			...
			$version = strtotime(mysql2date('d M Y H:i:s', $item->modified, false));
					...
					if($item && (current_user_can('administrator') || get_current_user_id()==$item->author) ) {
						$result = $wpdb->update(
							...
							array(
								...
								'data' => serialize($itemData),
								'config' => serialize($itemConfig),
								//'author' => get_current_user_id(), // do not change author
								//'date' => NULL,
								'modified' => current_time('mysql', 1)
							),
							array('id'=>$inputId));
			...
			switch($type) {
				...
				case 'import-items' : {
					...
					foreach($wpdb->get_results($sql) as $key => $item) {
						array_push($data['list'], array('id' => $item->ID, 'title' => $item->post_title, 'modified' => (mysql2date('d M Y H:i:s', $item->post_modified, false))));
					}
				} break;
